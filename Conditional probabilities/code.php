...
do {
    ...
    foreach($P as $name => $values) {
        ...
        switch($name) {
            case "A":
                //A = 1 - NOT A
                if($P["NOT A"] != null) $P[$name] = subFractions([1, 1], $P["NOT A"]);
                //A = A AND B / B GIVEN A
            ...
            case "A AND B":
                ...
                if($P["A AND NOT B"] != null && $P["A"] != null) $P[$name] = subFractions($P["A"], $P["A AND NOT B"]);
                //A AND B = B * A GIVEN B
                elseif($P["B"] != null && $P["A GIVEN B"] != null) $P[$name] = mulFractions($P["B"], $P["A GIVEN B"]);
                //A AND B = A * B GIVEN A
                elseif($P["A"] != null && $P["B GIVEN A"] != null) $P[$name] = mulFractions($P["A"], $P["B GIVEN A"]);
                //A AND B = B - NOT A AND B
                elseif($P["B"] != null && $P["NOT A AND B"] != null) $P[$name] = subFractions($P["B"], $P["NOT A AND B"]);
                break;
            case "A AND NOT B":
                //A AND NOT B = A - A AND B
                if($P["A AND B"] != null && $P["A"] != null) $P[$name] = subFractions($P["A"], $P["A AND B"]);
                //A AND NOT B = A GIVEN NOT B * NOT B
                elseif($P["A GIVEN NOT B"] != null && $P["NOT B"] != null) $P[$name] = mulFractions($P["A GIVEN NOT B"], $P["NOT B"]);
                //A AND NOT B = NOT B GIVEN A * A
                elseif($P["NOT B GIVEN A"] != null && $P["A"] != null) $P[$name] = mulFractions($P["NOT B GIVEN A"], $P["A"]);
                break;
            case "A GIVEN B":
                //A GIVEN B = "A AND B / B
                if($P["B"] != null && $P["A AND B"] != null) $P[$name] = divFractions($P["A AND B"], $P["B"]);
                //A GIVEN B = 1 - NOT A GIVEN B
                elseif($P["NOT A GIVEN B"] != null) $P[$name] = subFractions([1, 1], $P["NOT A GIVEN B"]);
                break;
            case "A GIVEN NOT B":
                //A GIVEN NOT B = A AND NOT B / NOT B
                if($P["A AND NOT B"] != null && $P["NOT B"] != null) $P[$name] = divFractions($P["A AND NOT B"], $P["NOT B"]);
                //A GIVEN NOT B = 1 - NOT A GIVEN NOT B
                elseif($P["NOT A GIVEN NOT B"] != null) $P[$name] = subFractions([1, 1], $P["NOT A GIVEN NOT B"]);
                break;
            case "B":
                //B = 1 - NOT B
                if($P["NOT B"] != null) $P[$name] = subFractions([1, 1], $P["NOT B"]);
                //B = A AND B / A GIVEN B
                elseif($P["A AND B"] != null && $P["A GIVEN B"] != null) $P[$name] = divFractions($P["A AND B"], $P["A GIVEN B"]);
                //B = A AND B + NOT A AND B
                elseif($P["A AND B"] != null && $P["NOT A AND B"] != null) $P[$name] = addFractions($P["A AND B"], $P["NOT A AND B"]);
                break;
            case "B GIVEN A":
                //B GIVEN A = A AND B / A
                if($P["A"] != null && $P["A AND B"] != null) $P[$name] = divFractions($P["A AND B"], $P["A"]);
                //B GIVEN A = 1 - NOT B GIVEN A
                elseif($P["NOT B GIVEN A"] != null) $P[$name] = subFractions([1, 1], $P["NOT B GIVEN A"]);
                break;
            case "B GIVEN NOT A":
                //B GIVEN NOT A = 1 - NOT B GIVEN NOT A
                if($P["NOT B GIVEN NOT A"] != null) $P[$name] = subFractions([1, 1], $P["NOT B GIVEN NOT A"]);
                //B GIVEN NOT A = NOT A AND B / NOT A
                elseif($P["NOT A AND B"] != null && $P["NOT A"] != null) $P[$name] = divFractions($P["NOT A AND B"], $P["NOT A"]);
                break;
            case "NOT A":
                //NOT A = 1 - A
            ...
            case "NOT A AND B":
                ...
                if($P["B"] != null && $P["A AND B"] != null) $P[$name] = subFractions($P["B"], $P["A AND B"]);
                //NOT A AND B = B GIVEN NOT A * NOT A
                elseif($P["B GIVEN NOT A"] != null && $P["NOT A"] != null) $P[$name] = mulFractions($P["B GIVEN NOT A"], $P["NOT A"]);
                //NOT A AND B = NOT A - NOT A AND NOT B
                elseif($P["NOT A"] != null && $P["NOT A AND NOT B"] != null) $P[$name] = subFractions($P["NOT A"], $P["NOT A AND NOT B"]);
                break;
            case "NOT A AND NOT B":
                //NOT A AND NOT B = NOT A - NOT A AND B
                if($P["NOT A"] != null && $P["NOT A AND B"] != null) $P[$name] = subFractions($P["NOT A"], $P["NOT A AND B"]);
                break;
            case "NOT A GIVEN B":
                //NOT A GIVEN B = 1 - A GIVEN B
                if($P["A GIVEN B"] != null) $P[$name] = subFractions([1, 1], $P["A GIVEN B"]);
                break;
            case "NOT A GIVEN NOT B":
                //NOT A GIVEN NOT B = 1 - A GIVEN NOT B
                if($P["A GIVEN NOT B"] != null) $P[$name] = subFractions([1, 1], $P["A GIVEN NOT B"]);
                break;
            case "NOT B":
                //NOT B = 1 - B
                if($P["B"] != null) $P[$name] = subFractions([1, 1], $P["B"]);
                //NOT B = A AND NOT B / A GIVEN NOT B
                elseif($P["A AND NOT B"] != null && $P["A GIVEN NOT B"] != null) $P[$name] = divFractions($P["A AND NOT B"], $P["A GIVEN NOT B"]);
                break;
            case "NOT B GIVEN A":
                //NOT B GIVEN A = 1 - B GIVEN A
                if($P["B GIVEN A"] != null) $P[$name] = subFractions([1, 1], $P["B GIVEN A"]);
                //NOT B GIVEN A = A AND NOT B / A
                elseif($P["A"] != null && $P["A AND NOT B"] != null) $P[$name] = divFractions($P["A AND NOT B"], $P["A"]);
                break;
            case "NOT B GIVEN NOT A":
                //NOT B GIVEN NOT A = 1 - B GIVEN NOT A
                if($P["B GIVEN NOT A"] != null) $P[$name] = subFractions([1, 1], $P["B GIVEN NOT A"]);
                break;
        }
    }
